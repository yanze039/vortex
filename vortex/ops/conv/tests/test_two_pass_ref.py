...
def test_ref_chunked_fwd(bs, seqlen, d, g, filter_size, dtype):
    ...
    x, B, C, h = setup_inputs(
        bs, seqlen, dg, g, filter_size, dtype, requires_grad=False
    )
...
def test_ref_chunked_bwd(bs, seqlen, d, g, filter_size, dtype):
    ...
    if not dx_passed:
        print_diff(
            dx, dx_ref, f"{bs=} {seqlen=} {d=} {g=} {filter_size=} {dtype=}, dx diff: "
        )

    dB_passed = torch.allclose(dB, dB_ref, atol=1e-4)
    if not dB_passed:
        print_diff(
            dB, dB_ref, f"{bs=} {seqlen=} {d=} {g=} {filter_size=} {dtype=}: dB diff: "
        )

    dC_passed = torch.allclose(dC, dC_ref, atol=1e-4)
    if not dC_passed:
        print_diff(
            dC, dC_ref, f"{bs=} {seqlen=} {d=} {g=} {filter_size=} {dtype=}: dC diff: "
        )
    ...
    if not dh_passed:
        print_diff(
            dh, dh_ref, f"{bs=} {seqlen=} {d=} {g=} {filter_size=} {dtype=}: dh diff: "
        )
...
def test_ref_chunked_bwd_store_reduce(bs, seqlen, d, g, filter_size, dtype):
    ...
    # Test grad
    *_, dT, dTc = gcg_two_pass_chunked_bwd(
        dy, x, y2, B, C, h, gl=chunk_size, return_dT=True
    )

    assert dT.shape == torch.Size(
        [g, chunk_size, chunk_size]
    ) and dTc.shape == torch.Size([g, chunk_size, chunk_size])
